        ...
        <tbody>
          {admins.map((admin) => (
            <tr key={admin._id}>
              ...
              <td>{admin.role}</td>
              <td>
                <button onClick={() => onEdit(admin)}style={{color:"white", backgroundColor:"green"}}>Edit</button>
                <button onClick={() => deleteAdmin(admin._id)} style={{color:"white", backgroundColor:"red"}}>Delete</button>
              </td>
            </tr>
          ))}
          {admins.length === 0 && (
